...
(function(){
  ...
  function openMatchRadarV2(fixtureId){
    console.log('ð¯ Match clicked - fixtureId:', fixtureId);
    ensureStyles();
    renderLoadingModal();
    ...
    if(window.__MATCH_CTX__ && window.__MATCH_CTX__.match) {
      ...
      console.log('ð¦ Match data found in context:', data);
  ...
  function classifyTrend(gfA, gaA, gfB, gaB) {
    ...
    if (t >= 10) return "TendÃªncia de 2+ gols";
    return "TendÃªncia de poucos gols";
  }
  ...
  function classifyDefense(ga) {
    ...
    if (n >= 5) return "Defesa instÃ¡vel";
  ...
  <div class="rt-statsv2">
    <div class="sv2-tabs" role="tablist" aria-label="Stats mode">
      <div class="sv2-tab ${mode==="last5"?"is-active":""}" data-sv2-mode="last5">Ãltimos 5 jogos</div>
    ...
    <div class="sv2-grid">
      <div class="sv2-card">
        <div class="sv2-card-h">
          ...
          <div class="sv2-pill">amostra: ${homeGames ?? "â"}</div>
        </div>

        <div class="sv2-card-b">
          <div class="sv2-kpis">
            <div class="sv2-kpi"><div class="k">Jogos</div><div class="v">${fmt(homeGames)}</div><div class="h">usados</div></div>
            <div class="sv2-kpi"><div class="k">GF</div><div class="v">${fmt(hGF)}</div><div class="h">gols prÃ³</div></div>
          ...
          <div class="sv2-compare">
            ...
            <div class="sv2-note">
              ObservaÃ§Ã£o: cartÃµes/escanteios sÃ³ aparecem quando o snapshot traz esses campos. Aqui mostramos o essencial (GF/GA/CS/FTS).
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="sv2-card">
        <div class="sv2-card-h">
          ...
          <div class="sv2-pill">amostra: ${awayGames ?? "â"}</div>
        </div>

        <div class="sv2-quick">
          <div class="qtitle">Leitura rÃ¡pida</div>
          <div class="sv2-qline"><div class="sv2-qk">TendÃªncia</div><div class="sv2-qv">${trend}</div></div>
          ...
          <div class="sv2-note">
            Se algum time vier com âââ, o snapshot ainda nÃ£o trouxe dados suficientes para essa amostra.
          </div>
        </div>
      </div>
    </div>
  ...
  function renderStatsTab(ov, data){
    ...
    panel.innerHTML = `<div class="mr-v2-empty">${t('match_radar.loading_stats', 'Carregando estatÃ­sticas...')}</div>`;
  ...
  function renderStatsTabLegacy(ov, data, reason){
    ...
    if((!hasGoalsData && !hasFormData) || (allGoalsZero && !hasFormData)) {
      ...
      panel.innerHTML = `<div class="mr-v2-empty">${t('match_radar.no_stats', 'EstatÃ­sticas indisponÃ­veis para este jogo')}</div>`;
      return;
    }
    ...
    const lastMatches = `${t('match_radar.form_window', `Ãltimos ${goalsWindow} jogos`).replace('{n}', goalsWindow)}`;
    html += lastMatches !== 'match_radar.form_window' ? lastMatches : `Ãltimos ${goalsWindow} jogos`;
    html += `</div>`;
...
function renderComplianceFooter(lang){
  ...
  if(!foot) return;
  const labels = {
    en:{note:"Informational content â¢ We are not a bookmaker â¢ 18+"},
    pt:{note:"ConteÃºdo informativo â¢ NÃ£o somos casa de apostas â¢ +18"},
    es:{note:"Contenido informativo â¢ No somos casa de apuestas â¢ 18+"},
    fr:{note:"Contenu informatif â¢ Pas un bookmaker â¢ 18+"},
    de:{note:"Info-Inhalt â¢ Kein Buchmacher â¢ 18+"}
  }[lang] || {note:"Informational content â¢ We are not a bookmaker â¢ 18+"};
...
function ensureTopSearch(t){
  ...
  wrap.innerHTML = `
    <span class="top-search-ico" aria-hidden="true">ð</span>
...
function renderTop3(t, data){
  ...
  cards.forEach((card, idx)=>{
    ...
    if(!item){
      ...
      meta.innerHTML = "";
      if(suggestionEl) suggestionEl.textContent = "â";
      lock.innerHTML = "";
...
function groupByCountryCompetition(matches){
  ...
  for (const m of sorted) {
    const country = String(m.country || "â").trim() || "â";
    const comp = String(competitionDisplay(m.competition, m.country, LANG) || "â").trim() || "â";
...
function renderCalendar(t, todayMatches, tomorrowMatches, meta, viewMode, query, activeTabType){
  console.log('ð renderCalendar called:');
  ...
  function renderMatchRow(m){
    ...
    const scoreText = hasScore ? `${goalHomeRaw} - ${goalAwayRaw}` : "â";
    ...
    const market = localizeMarket(m.suggestion_free, t) || "â";
    ...
    row.innerHTML = `
      ...
      <div class="rt-cal-actions">
        ...
        <button class="rt-match-more-btn rt-cal-more" type="button" title="${escAttr(moreTitle)}" aria-label="${escAttr(moreTitle)}" ${moreAttrs}>ï¼</button>
      </div>
    `;
  ...
  for (const cg of countryGroups) {
    ...
    const countryName = String(cg.country || "â");
    ...
    countryBox.innerHTML = `
      <div class="rt-cal-country-head">
        <button class="rt-cal-country-toggle ${isCollapsed ? "" : "is-open"}" data-country="${escAttr(countryName)}" type="button" aria-expanded="${isCollapsed ? "false" : "true"}" aria-label="${escAttr(countryName)}">
          ...
          <span class="rt-cal-country-chevron" aria-hidden="true">â¾</span>
        </button>
      </div>
...
async function loadRadarDay() {
  ...
  console.log('ð¡ loadRadarDay() returned:', {
...
async function loadCalendar2D() {
  ...
  try {
    ...
    if ((!Array.isArray(data.today) || data.today.length === 0) || isWorkerDataStale()) {
      console.warn('â ï¸ Worker data is stale or empty, falling back to static calendar file');
      ...
      try {
        ...
        if (staticFallback.ok) {
          ...
          console.warn('â Using fresh static calendar');
...
async function renderCompetitionStandings(leagueId, season, fallbackMatches = []){
  ...
  if(seasonResolved === null){
    ...
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "ClassificaÃ§Ã£o indisponÃ­vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">${escAttr(debug)}</div></div>`;
  }

  if(!standingsFile){
    return `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr(T.standings_unavailable || "ClassificaÃ§Ã£o indisponÃ­vel no momento.")} (sem leagueId/season)</div>`;
  }

  if(!entry || !entry.standings){
    const debugInfo = `Liga ${lid} / Season ${seasonResolved} / Arquivo: ${standingsFile} / Entry: ${entry ? 'existe' : 'NÃO EXISTE'} / Standings: ${entry?.standings ? 'existe' : 'NÃO EXISTE'}`;
    console.warn("[STANDINGS] Entry check failed:", debugInfo);
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "ClassificaÃ§Ã£o indisponÃ­vel no momento.")}</div><div style="font-size:0.75em;margin-top:8px;opacity:0.6;font-family:monospace;word-break:break-all;">${escAttr(debugInfo)}</div></div>`;
  }
  ...
  if (!standings || standings.schemaVersion !== 1 || !Array.isArray(standings.standings)) {
    ...
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "ClassificaÃ§Ã£o indisponÃ­vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Snapshot incompatÃ­vel</div></div>`;
  }
  ...
  if(standings.standings.length === 0){
    const reason = standings.meta?.dataStatus === 'empty' 
      ? 'API sem dados para esta competiÃ§Ã£o/temporada'
      : `Arquivo: ${standingsFile}`;
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "ClassificaÃ§Ã£o indisponÃ­vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">${escAttr(reason)}</div></div>`;
  }

  const table = standings.standings;
  const sampleTeams = Array.isArray(table)
    ? table.slice(0, 5).map(row => row?.team?.name || row?.team || row?.name || "â")
    : [];
  ...
  if(isDummy){
    ...
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "ClassificaÃ§Ã£o indisponÃ­vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Aguardando dados reais da API...</div></div>`;
  }

  // Render table
  const rows = table.map((row, idx) => {
    const rank = row.rank || row.position || (idx + 1);
    const team = row.team?.name || row.team || row.name || "â";
...
async function renderCompetitionStats(leagueId, season, fallbackMatches = []){
  ...
  if(seasonResolved === null){
    const debug = `leagueId=${String(leagueId)} kickoff=${String(kickoffUTC || "")}`;
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "EstatÃ­sticas indisponÃ­veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">${escAttr(debug)}</div></div>`;
  }

  if(compstatsFile){
    if(!entry){
      return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "EstatÃ­sticas indisponÃ­veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Arquivo: ${escAttr(compstatsFile)}</div></div>`;
    }
    if(!entry.compstats){
      return `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr("EstatÃ­sticas ainda nÃ£o disponÃ­veis para esta competiÃ§Ã£o/temporada.")}</div>`;
    }
    ...
    if(compStats && Number(compStats.schemaVersion) !== 1){
      ...
      return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "EstatÃ­sticas indisponÃ­veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Snapshot incompatÃ­vel</div></div>`;
    }
    ...
    if(compStats && (!compStats.metrics || Object.keys(compStats.metrics).length === 0)){
      ...
      return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "EstatÃ­sticas indisponÃ­veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Aguardando dados reais da API...</div></div>`;
    }
  ...
  return `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr(T.no_stats_available || "Sem dados de estatÃ­sticas.")} (sem leagueId/season)</div>`;
}
...
function renderCompStatsDisplay(compStats){
  ...
  const stats = [
    ...
    { label: T.cards_per_game || "CartÃµes por jogo", value: fmt(metrics.cards_avg) },
  ];

  const optionalStats = [];
  if(metrics.possession_avg !== null && metrics.possession_avg !== undefined){
    optionalStats.push({ label: T.possession_avg || "Posse mÃ©dia", value: fmt(metrics.possession_avg) + "%" });
  }
  ...
  return `
    <div class="panel" style="margin-bottom:12px;">
      ...
      <div style="opacity:.85;font-size:.9em;">
        ...
        <div>${escAttr(T.with_stats || "Com estatÃ­sticas")}: <b>${escAttr(String(sample.fixtures_with_stats || 0))}</b></div>
        <div>${escAttr(T.period_label || "PerÃ­odo")}: <b>${escAttr(T.generated_at || compStats.generated_at_utc || "â")}</b></div>
      </div>
    </div>

    <div class="panel">
      <div class="panel-title">${escAttr(T.statistics_label || "EstatÃ­sticas")}</div>
...
async function openModal(type, value){
  ...
  if(type === "competition" || type === "country"){
    title.textContent = T.unavailable_title || "IndisponÃ­vel";
    body.innerHTML = `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr(T.unavailable_text || "ConteÃºdo nÃ£o disponÃ­vel no modo Free.")}</div>`;
  ...
  if(type === "about"){
    ...
    body.innerHTML = `
      ...
      <div class="mnote">
        ${escAttr(T.free_includes || "FREE: sugestÃ£o + risco + forma + gols.")}
      </div>
    `;
...
function ensureSidebar(t, lang){
  ...
  aside.innerHTML = `
    ...
    <nav class="side-nav" aria-label="Navigation">
      <a class="side-item ${here==="day"?"active":""}" href="${nav.day}"><span class="i">â¡</span><span>${escAttr(t.nav_day || "Daily Radar")}</span></a>
    </nav>

    <div class="side-divider"></div>

    <nav class="side-nav" aria-label="Info">
      <a class="side-item" href="${p.how}"><span class="i">ð§­</span><span>${escAttr(t.how_link || "How it works")}</span></a>
      <a class="side-item" href="${p.about}"><span class="i">â¹ï¸</span><span>${escAttr(t.about_link || "About")}</span></a>
      <a class="side-item" href="${p.contact}"><span class="i">âï¸</span><span>${escAttr(t.contact_link || "Contact")}</span></a>
    </nav>
...
async function init(){
  ...
  if (!radar || isMockDataset(radar) || (Array.isArray(radar.highlights) && radar.highlights.length===0 && Array.isArray(radar.matches) && radar.matches.length===0)) {
    console.warn('â ï¸ Radar data missing or empty');
  ...
  function rerender(){
    ...
    console.log('ð renderCalendar with merged today:', todayWithRadar.length, 'matches (including', radarMatches.length, 'radar highlights)');
