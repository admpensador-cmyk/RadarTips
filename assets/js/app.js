...
async function init(){
  ...
  // === MATCH CARD CLICK (ABRIR MATCH RADAR) ===
  qsa("[data-fixture-id]").forEach(card=>{
    if(card.dataset.boundMatch === "1") return;
    card.dataset.boundMatch = "1";

    card.addEventListener("click", (e)=>{

      // bloquear elementos interativos
      if(e.target.closest("a, button, img, svg, [data-open]")) return;

      const fixtureId = card.getAttribute("data-fixture-id");
      if(!fixtureId) return;

      // encontrar match no dataset
      const match = CAL_MATCHES.find(m =>
        String(m.fixture_id || m.id || m.fixture || m.fixtureId) === String(fixtureId)
      );

      if(!match){
        console.warn("Match não encontrado no dataset:", fixtureId);
        return;
      }

      const key = matchKey(match);

      // abrir modal nativo (não usar openMatchRadarV2)
      openModal("match", key);
    });
