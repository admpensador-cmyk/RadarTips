...
async function init(){
  ...
  // Date strip - initialize days and activeDate FIRST
  ...
  // Adjust activeDate if today isn't in the calendar data
  if(CAL_MATCHES.length){
    ...
    if(uniq.length && !uniq.includes(activeDate)){
      // Today's date not in calendar data - use first available date instead
      activeDate = uniq[0];
      
      if (DEBUG_CAL) {
        console.warn('[CAL] Today not in data, using first available date:', {
          today_attempted: new Intl.DateTimeFormat("en-CA", {year:"numeric", month:"2-digit", day:"2-digit"}).format(days[0]),
          activeDate_set_to: activeDate,
