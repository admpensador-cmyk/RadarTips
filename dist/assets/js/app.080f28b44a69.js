...
// Load calendar and compute today/tomorrow in user's local timezone.
async function loadCalendar2D() {
  ...
  try {
    ...
    const normalized = normalizeCalendarByLocalDay(data);
...
function localDateKeyFromIso(isoUtc) {
...
function addDaysToLocalDateKey(baseKey, days) {
  const [y, m, d] = String(baseKey || '').split('-').map(Number);
...
function splitCalendarByLocalDay(matches) {
  const todayKey = localDateKeyFromIso(new Date().toISOString());
  const tomorrowKey = addDaysToLocalDateKey(todayKey, 1);
  const today = [];
  const tomorrow = [];

  for (const m of Array.isArray(matches) ? matches : []) {
    const dayKey = localDateKeyFromIso(m?.kickoff_utc);
    if (!dayKey) continue;
    if (dayKey === todayKey) today.push(m);
    else if (dayKey === tomorrowKey) tomorrow.push(m);
  }

  return { todayKey, tomorrowKey, today, tomorrow };
}

function normalizeCalendarByLocalDay(data) {
  const baseMatches = Array.isArray(data?.matches)
    ? data.matches
    : flattenCalendarMatches(data || {});
  const split = splitCalendarByLocalDay(baseMatches);
  ...
  return {
    ...
    meta: {
      ...(data?.meta || {}),
      day_basis: 'local',
      today_key: split.todayKey,
      tomorrow_key: split.tomorrowKey,
      today: split.todayKey,
      tomorrow: split.tomorrowKey,
      source_matches_count: baseMatches.length
    },
    matches: baseMatches,
    today: split.today,
