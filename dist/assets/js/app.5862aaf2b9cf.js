...
(function(){
    ...
    renderShell(appState){
      ...
      const shellInner = `
          <div class="mr-v2-head">
            ...
            <div class="mr-v2-actions">${fullAction}<button class="mr-v2-close">├ù</button></div>
          </div>
      ...
      if(tab === 'markets') {
        ...
        appState.content.innerHTML = `
          <div class="mr-v2-details">
            ...
            <div class="mr-v2-detail-row"><strong>${t('match_radar.details.league', 'Liga')}</strong><span>${leagueName || 'ÔÇö'}</span></div>
            <div class="mr-v2-detail-row"><strong>${t('match_radar.details.country', 'Pais')}</strong><span>${country || 'ÔÇö'}</span></div>
            <div class="mr-v2-detail-row"><strong>${t('match_radar.details.kickoff', 'Kickoff UTC')}</strong><span>${kickoff || 'ÔÇö'}</span></div>
          </div>
        `;
      }
    ...
    try{
      ...
      if(Array.isArray(arr)){
        markets = arr.map(entry => {
          ...
          const line = entry.line || entry.lineValue || entry.threshold || entry.stake || entry.odds || entry.value || entry.betValue || entry.amount || entry.entry || pick || 'ÔÇö';
  ...
  function renderLoadingModal(mode = 'sidebar'){
    ...
    const shellInner = `
        <div class="mr-v2-head">
          ...
          <button class="mr-v2-close">├ù</button>
        </div>
  ...
  function renderEmpty(){
    ...
    if(modal) modal.innerHTML = '<div class="mr-v2-empty">Sem dados dispon├¡veis</div>';
  }
  ...
  const REASON_KEY_MAP = {
    "Perfil mais controlado de gols, mas risco alto por oscila├º├Áes e contexto.": "match_radar.reason.controlled_goals_high_risk",
    "For├ºa relativa e forma favorecem prote├º├úo, mas risco alto por imprevisibilidade do resultado.": "match_radar.reason.relative_strength_protection",
    "Se houver gol cedo, a linha tende a ficar viva (risco m├®dio).": "match_radar.reason.early_goal_live_line",
    "Favoritismo leve com prote├º├úo do empate, mas risco alto por margem curta.": "match_radar.reason.slight_favorite_dnb",
  ...
  function renderMarketsTab(ov, data){
    ...
    if(!arr || arr.length===0){ 
      panel.innerHTML = `<div class="mr-v2-empty">${t('match_radar.empty', 'Sem dados dispon├¡veis')}</div>`; 
      return; 
    }
    ...
    const rows = arr.slice().map(m=>{
      ...
      const riskStr = risk !== null ? `${Math.round(risk * 100)}%` : 'ÔÇö';
      
      // Format odd_fair with 2 decimals
      const oddStr = odd_fair !== null ? Number(odd_fair).toFixed(2) : 'ÔÇö';
      
      const market = escapeHtml(m.market || 'ÔÇö');
      const line = escapeHtml(m.line || 'ÔÇö');
      const reasonText = resolveReasonText(m.reason, m) || 'ÔÇö';
  ...
  function classifyTrend(gfA, gaA, gfB, gaB) {
    ...
    if (t >= 10) return "Tend├¬ncia de 2+ gols";
    return "Tend├¬ncia de poucos gols";
  }
  ...
  function classifyDefense(ga) {
    ...
    if (n >= 5) return "Defesa inst├ível";
  ...
  function renderStatsV2(api, ctx) {
    ...
    const formatSampleLabel = (meta, games) => {
      ...
      if(games === null) return 'amostra: ÔÇö';
  ...
  <div class="rt-statsv2">
    <div class="sv2-tabs" role="tablist" aria-label="Stats mode">
      <div class="sv2-tab ${mode==="last5"?"is-active":""}" data-sv2-mode="last5">├Ültimos 5 jogos</div>
    ...
    <div class="sv2-grid">
      <div class="sv2-card">
        ...
        <div class="sv2-card-b">
          <div class="sv2-kpis">
            ...
            <div class="sv2-kpi"><div class="k">GF</div><div class="v">${fmt(hGF)}</div><div class="h">gols pr├│</div></div>
          ...
          <div class="sv2-compare">
            ...
            <div class="sv2-note">
              Observa├º├úo: cart├Áes/escanteios s├│ aparecem quando o snapshot traz esses campos. Aqui mostramos o essencial (GF/GA/CS/FTS).
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="sv2-card">
        ...
        <div class="sv2-quick">
          <div class="qtitle">Leitura r├ípida</div>
          <div class="sv2-qline"><div class="sv2-qk">Tend├¬ncia</div><div class="sv2-qv">${trend}</div></div>
          ...
          <div class="sv2-note">
            Se algum time vier com ÔÇ£ÔÇöÔÇØ, o snapshot ainda n├úo trouxe dados suficientes para essa amostra.
          </div>
        </div>
      </div>
    </div>
  ...
  function renderStatsTab(ov, data, prefetchedStats){
    ...
    panel.innerHTML = `<div class="mr-v2-stats-loading">${t('match_radar.loading_stats', 'Carregando estat├¡sticas...')}</div>`;
  ...
  function renderStatsTabLegacy(ov, data, reason){
    ...
    if((!hasGoalsData && !hasFormData) || (allGoalsZero && !hasFormData)) {
      ...
      panel.innerHTML = `<div class="mr-v2-empty">${t('match_radar.no_stats', 'Estat├¡sticas indispon├¡veis para este jogo')}</div>`;
      return;
    }

    const homeName = data.home?.name || data.home || 'ÔÇö';
    const awayName = data.away?.name || data.away || 'ÔÇö';
    let html = `<div class="mr-stats-container" style="padding:20px;">`;
    html += `<div style="font-size:0.9em;color:#888;margin-bottom:15px;">`;
    const lastMatches = `${t('match_radar.form_window', `├Ültimos ${goalsWindow} jogos`).replace('{n}', goalsWindow)}`;
    html += lastMatches !== 'match_radar.form_window' ? lastMatches : `├Ültimos ${goalsWindow} jogos`;
    html += `</div>`;
    ...
    if(hasGoalsData) {
      ...
      html += `<div><span style="color:#999;">${t('match_radar.stats.avg_gf', 'M├®dia GF')}:</span> <span style="color:#fff;font-weight:500;">${avgGF}</span></div>`;
      html += `<div><span style="color:#999;">${t('match_radar.stats.avg_ga', 'M├®dia GA')}:</span> <span style="color:#fff;font-weight:500;">${avgGA}</span></div>`;
      html += `<div><span style="color:#999;">${t('match_radar.stats.avg_total', 'Total m├®dio')}:</span> <span style="color:#fff;font-weight:500;">${avgTotal}</span></div>`;
      html += `</div>`;
    }
    html += `</div>`;
...
function renderComplianceFooter(lang){
  ...
  if(!foot) return;
  const labels = {
    en:{note:"Informational content ÔÇó We are not a bookmaker ÔÇó 18+"},
    pt:{note:"Conte├║do informativo ÔÇó N├úo somos casa de apostas ÔÇó +18"},
    es:{note:"Contenido informativo ÔÇó No somos casa de apuestas ÔÇó 18+"},
    fr:{note:"Contenu informatif ÔÇó Pas un bookmaker ÔÇó 18+"},
    de:{note:"Info-Inhalt ÔÇó Kein Buchmacher ÔÇó 18+"}
  }[lang] || {note:"Informational content ÔÇó We are not a bookmaker ÔÇó 18+"};
...
function marketRiskLabel(r){
  ...
  if(v==="volatile") return (T.risk_volatile || "Vol├ítil");
  return (T.risk_med || "M├®dio");
}
...
// Canonical Radar Day runtime endpoints.
const V1_API_BASE = "/api/v1";
const RADAR_DAY_ENDPOINT = "/api/v1/radar_day";
const CALENDAR_2D_ENDPOINT = "/api/v1/calendar_2d";
...
async function loadV1JSON(file, fallback){
  ...
  if(normalized === "radar_day.json"){
    return await loadJSON(RADAR_DAY_ENDPOINT, fallback);
  }
  if(normalized === "calendar_2d.json" || normalized === "calendar_day.json"){
    return await loadJSON(CALENDAR_2D_ENDPOINT, fallback);
  }
  return await loadJSON(`${V1_API_BASE}/${normalized}`, fallback);
}
    ...
    for(const el of els){
      ...
      if(pill){
        ...
        if(isFinal){
          ...
          pill.querySelector(".txt").textContent = t.pending_label || "ÔÇö";
        }
      }
...
function showUpdatingMessage(container){
  ...
  container.innerHTML = `
    <div class="empty-state">
      <div class="empty-title">Updating match dataÔÇª</div>
      <div class="empty-sub">WeÔÇÖre generating todayÔÇÖs radar. Please refresh in a few minutes.</div>
    </div>`;
}
...
function ensureTopSearch(t){
  ...
  wrap.innerHTML = `
    <span class="top-search-ico" aria-hidden="true">­ƒöÄ</span>
...
function riskTone(riskRaw){
  ...
  if(risk.includes("med") || risk.includes("medio") || risk.includes("m├®dio")) return "medium";
  return "risk";
}
...
function renderPremiumDayPanels(t, matches, highlights, activeTab){
  const subtitle = activeTab === "tomorrow"
    ? (t.hero_sub_tomorrow || "Leitura priorit├íria do dia selecionado")
    : (t.hero_sub_day || "Leitura priorit├íria do dia selecionado");
...
function renderTop3(t, data){
  ...
  cards.forEach((card, idx)=>{
    ...
    if(!item){
      ...
      meta.innerHTML = "";
      if(suggestionEl) suggestionEl.textContent = "ÔÇö";
      lock.innerHTML = "";
    ...
    lock.innerHTML = `
      ...
      <div class="rt-card-confidence">
        <div class="rt-card-confidence-head"><small>${escAttr(t.confidence_label || "Confian├ºa")}</small></div>
...
function competitionDisplay(rawComp, country, lang){
  ...
  if(n === "serie a"){
    if(c === "brazil") return pick("Brasileir├úo S├®rie A","Brazil Serie A","Serie A (Brasil)","S├®rie A (Br├®sil)","Serie A (Brasilien)");
    if(c === "italy")  return pick("Serie A (It├ília)","Serie A (Italy)","Serie A (Italia)","Serie A (Italie)","Serie A (Italien)");
  }
  if(n === "serie b"){
    if(c === "brazil") return pick("Brasileir├úo S├®rie B","Brazil Serie B","Serie B (Brasil)","S├®rie B (Br├®sil)","Serie B (Brasilien)");
    if(c === "italy")  return pick("Serie B (It├ília)","Serie B (Italy)","Serie B (Italia)","Serie B (Italie)","Serie B (Italien)");
  }
  if(n === "serie c"){
    if(c === "brazil") return pick("Brasileir├úo S├®rie C","Brazil Serie C","Serie C (Brasil)","S├®rie C (Br├®sil)","Serie C (Brasilien)");
  }
...
function groupByCountryCompetition(matches){
  ...
  for (const m of sorted) {
    const country = String(m.country || "ÔÇö").trim() || "ÔÇö";
    const comp = String(competitionDisplay(m.competition, m.country, LANG) || "ÔÇö").trim() || "ÔÇö";
...
function renderMarketsTable(markets){
  ...
  const rows = arr.map(m=>{
    ...
    const riskLabel = (risk==="volatile") ? (T.risk_volatile || "Vol├ítil")
      : (risk==="low") ? (T.risk_low || "Baixo")
      : (risk==="high") ? (T.risk_high || "Alto")
      : (T.risk_med || "M├®dio");

    const confPct = Math.round((Number(m?.confidence || 0) * 100));
    const confTxt = `${escAttr(T.confidence_label || "Confian├ºa")}: <b>${confPct}%</b>`;
...
function renderSuggestions(match){
  ...
  if(!Array.isArray(markets) || markets.length===0){
    return `<div class="smallnote">${escAttr(T.no_markets || "Sem sugest├Áes dispon├¡veis.")}</div>`;
  }
      ...
      <div class="suggestion-row">
        ...
        <div class="suggestion-right">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
            ...
            <span style="opacity:.85;font-size:13px;margin-left:auto">${escAttr(T.confidence_label || 'Confian├ºa')}: <b>${conf}%</b></span>
          </div>
...
function renderStats(match){
  ...
  const rows = metrics.map(met=>{
    ...
    const lnum = (L===null||isNaN(L)) ? 'ÔÇö' : (Number.isFinite(L) ? (Math.round(L*100)/100) : 'ÔÇö');
    const rnum = (R===null||isNaN(R)) ? 'ÔÇö' : (Number.isFinite(R) ? (Math.round(R*100)/100) : 'ÔÇö');
    ...
    return `
      <div class="stat-row">
        ...
        <div class="stat-values">${escAttr(String(lnum))} ÔÇó ${escAttr(String(rnum))}</div>
      </div>
    `;
  }).filter(Boolean).join("");

  if(!rows) return `<div class="smallnote">${escAttr(T.no_stats || 'Estat├¡sticas indispon├¡veis.')}</div>`;
...
function renderCalendar(t, todayMatches, tomorrowMatches, meta, viewMode, query, activeTabType){
  console.log('­ƒôà renderCalendar called:');
  ...
  const tomorrowLabel = t.tab_tomorrow || "Amanh├ú";
  ...
  const signalsItems = [
    ...
    signalCard("Highest Tempo", highestTempo, (m)=> {
      ...
      return `M├®dia ofensiva combinada: ${total.toFixed(1)}`;
    }, "tempo"),
    signalCard("Most Balanced Match", mostBalanced, (m)=> {
      ...
      return `Diferen├ºa de forma: ${diff.toFixed(1)}`;
    }, "balanced")
  ].filter(Boolean);
  ...
  if(!signalsHtml.length){
    signalsColumn.innerHTML += `<p class="rt-day-signals-empty">Sem sinal forte dispon├¡vel hoje</p>`;
  }
  ...
  if(!filtered.length){
    ...
    if (!hasAnyMatches) {
      ...
      title = active === "today" 
        ...
        : (t.no_matches_tomorrow || "Sem jogos para amanh├ú");
  ...
  function renderMatchRow(m){
    ...
    const scoreText = hasScore ? `${goalHomeRaw} - ${goalAwayRaw}` : "ÔÇö";
    ...
    const market = localizeMarket(m.suggestion_free, t) || "ÔÇö";
  ...
  for (const cg of countryGroups) {
    ...
    const countryName = String(cg.country || "ÔÇö");
    ...
    countryBox.innerHTML = `
      <div class="rt-cal-country-head">
        <button class="rt-cal-country-toggle ${isCollapsed ? "" : "is-open"}" data-country="${escAttr(countryName)}" type="button" aria-expanded="${isCollapsed ? "false" : "true"}" aria-label="${escAttr(countryName)}">
          ...
          <span class="rt-cal-country-chevron" aria-hidden="true">Ôû¥</span>
        </button>
      </div>
...
async function loadRadarDay() {
  const radar = await loadJSON(RADAR_DAY_ENDPOINT, { highlights: [] });
  console.log('­ƒôí loadRadarDay() returned:', {
...
async function getRadarDay() {
  ...
  try {
    const data = await loadJSON(RADAR_DAY_ENDPOINT, null);
    if (data) {
      cache.data = data;
      cache.loadedAt = now;
      return data;
...
async function loadCalendar2D() {
  ...
  try {
    ...
    const url = `${CALENDAR_2D_ENDPOINT}?ts=${Date.now()}`;
    let response = await fetch(url, { cache: 'no-store' });

    if (!response.ok && response.status === 400) {
      console.warn('loadCalendar2D validation error, retrying default timezone', { tz, status: response.status });
      const fallbackUrl = `${CALENDAR_2D_ENDPOINT}?tz=${encodeURIComponent('America/Sao_Paulo')}&ts=${Date.now()}`;
...
function getRadarTop3ForTab(cal2d, tabType, radarFallback) {
  ...
  // HARDENING: Do NOT use stale fallback data as valid Top 3
  // Only use radarFallback as last resort if matches are current (within tolerance)
  if (Array.isArray(radarFallback?.highlights)) {
    const now = Date.now();
    const currentMatches = radarFallback.highlights.filter(m => {
      if (!m?.kickoff_utc) return false;
      const kickoffMs = new Date(String(m.kickoff_utc)).getTime();
      return kickoffMs > now;  // Only matches in future
    });
    if (currentMatches.length) return currentMatches.slice(0, 3);
    // If only past matches in fallback, return empty (degraded state)
    return [];
  }
  return [];
}
...
async function renderCompetitionStandings(leagueId, season, fallbackMatches = []){
  ...
  if(seasonResolved === null){
    ...
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "Classifica├º├úo indispon├¡vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">${escAttr(debug)}</div></div>`;
  }

  if(!standingsFile){
    return `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr(T.standings_unavailable || "Classifica├º├úo indispon├¡vel no momento.")} (sem leagueId/season)</div>`;
  }

  if(!entry || !entry.standings){
    const debugInfo = `Liga ${lid} / Season ${seasonResolved} / Arquivo: ${standingsFile} / Entry: ${entry ? 'existe' : 'N├âO EXISTE'} / Standings: ${entry?.standings ? 'existe' : 'N├âO EXISTE'}`;
    console.warn("[STANDINGS] Entry check failed:", debugInfo);
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "Classifica├º├úo indispon├¡vel no momento.")}</div><div style="font-size:0.75em;margin-top:8px;opacity:0.6;font-family:monospace;word-break:break-all;">${escAttr(debugInfo)}</div></div>`;
  }
  ...
  if (!standings || standings.schemaVersion !== 1 || !Array.isArray(standings.standings)) {
    ...
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "Classifica├º├úo indispon├¡vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Snapshot incompat├¡vel</div></div>`;
  }
  ...
  if(standings.standings.length === 0){
    const reason = standings.meta?.dataStatus === 'empty' 
      ? 'API sem dados para esta competi├º├úo/temporada'
      : `Arquivo: ${standingsFile}`;
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "Classifica├º├úo indispon├¡vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">${escAttr(reason)}</div></div>`;
  }

  const table = standings.standings;
  const sampleTeams = Array.isArray(table)
    ? table.slice(0, 5).map(row => row?.team?.name || row?.team || row?.name || "ÔÇö")
    : [];
  ...
  if(isDummy){
    ...
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.standings_unavailable || "Classifica├º├úo indispon├¡vel no momento.")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Aguardando dados reais da API...</div></div>`;
  }

  // Render table
  const rows = table.map((row, idx) => {
    const rank = row.rank || row.position || (idx + 1);
    const team = row.team?.name || row.team || row.name || "ÔÇö";
...
async function renderCompetitionStats(leagueId, season, fallbackMatches = []){
  ...
  if(seasonResolved === null){
    const debug = `leagueId=${String(leagueId)} kickoff=${String(kickoffUTC || "")}`;
    return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "Estat├¡sticas indispon├¡veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">${escAttr(debug)}</div></div>`;
  }

  if(compstatsFile){
    if(!entry){
      return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "Estat├¡sticas indispon├¡veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Arquivo: ${escAttr(compstatsFile)}</div></div>`;
    }
    if(!entry.compstats){
      return `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr("Estat├¡sticas ainda n├úo dispon├¡veis para esta competi├º├úo/temporada.")}</div>`;
    }
    ...
    if(compStats && Number(compStats.schemaVersion) !== 1){
      ...
      return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "Estat├¡sticas indispon├¡veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Snapshot incompat├¡vel</div></div>`;
    }
    ...
    if(compStats && (!compStats.metrics || Object.keys(compStats.metrics).length === 0)){
      ...
      return `<div class="smallnote" style="padding:20px;text-align:center;"><div>${escAttr(T.no_stats_available || "Estat├¡sticas indispon├¡veis")}</div><div style="font-size:0.85em;margin-top:6px;opacity:0.7;">Aguardando dados reais da API...</div></div>`;
    }
  ...
  return `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr(T.no_stats_available || "Sem dados de estat├¡sticas.")} (sem leagueId/season)</div>`;
}
...
function renderCompStatsDisplay(compStats){
  ...
  const stats = [
    ...
    { label: T.cards_per_game || "Cart├Áes por jogo", value: fmt(metrics.cards_avg) },
  ];

  const optionalStats = [];
  if(metrics.possession_avg !== null && metrics.possession_avg !== undefined){
    optionalStats.push({ label: T.possession_avg || "Posse m├®dia", value: fmt(metrics.possession_avg) + "%" });
  }
  ...
  return `
    <div class="panel" style="margin-bottom:12px;">
      ...
      <div style="opacity:.85;font-size:.9em;">
        ...
        <div>${escAttr(T.with_stats || "Com estat├¡sticas")}: <b>${escAttr(String(sample.fixtures_with_stats || 0))}</b></div>
        <div>${escAttr(T.period_label || "Per├¡odo")}: <b>${escAttr(T.generated_at || compStats.generated_at_utc || "ÔÇö")}</b></div>
      </div>
    </div>

    <div class="panel">
      <div class="panel-title">${escAttr(T.statistics_label || "Estat├¡sticas")}</div>
...
async function openModal(type, value){
  ...
  if(type === "competition" || type === "country"){
    title.textContent = T.unavailable_title || "Indispon├¡vel";
    body.innerHTML = `<div class="smallnote" style="padding:20px;text-align:center;">${escAttr(T.unavailable_text || "Conte├║do n├úo dispon├¡vel no modo Free.")}</div>`;
  ...
  if(type === "about"){
    ...
    body.innerHTML = `
      <div class="panel">
        ...
        <div class="panel">
          <div class="panel-title" ${tipAttr(goalsTip)}>${escAttr(T.goals_title || "Gols (├║ltimos 5)")}</div>
          <div class="goals">
      ...
      <div class="mnote">
        ${escAttr(T.free_includes || "FREE: sugest├úo + risco + forma + gols.")}
      </div>
    `;
    ...
    if(!list.length){
      ...
      body.innerHTML = `
        <div class="smallnote" style="padding:20px;text-align:center;">
          <div style="font-size:1.1em;margin-bottom:10px;">${escAttr(T.match_not_found_title || "Dados do jogo ainda n├úo dispon├¡veis")}</div>
    ...
    body.innerHTML = `
      <div class="mhead">
        <div class="mmeta">
          ...
          <div class="smallnote">${escAttr(T.free_includes || "FREE: sugest├úo + risco + forma + gols.")}</div>
        </div>
        <div class="mbadges">
      ...
      <div class="tab-buttons">
        <button class="tab-btn active" data-tab="suggestions">${escAttr(T.suggestions_tab || "Sugest├Áes")}</button>
        <button class="tab-btn" data-tab="stats">${escAttr(T.stats_tab || "Estat├¡sticas")}</button>
      </div>
...
function ensureSidebar(t, lang){
  ...
  aside.innerHTML = `
    ...
    <nav class="side-nav" aria-label="Navigation">
      <a class="side-item ${here==="day"?"active":""}" href="${nav.day}"><span class="i">ÔÜí</span><span>${escAttr(t.nav_day || "Daily Radar")}</span></a>
    </nav>

    <div class="side-divider"></div>

    <nav class="side-nav" aria-label="Info">
      <a class="side-item" href="${p.how}"><span class="i">­ƒº¡</span><span>${escAttr(t.how_link || "How it works")}</span></a>
      <a class="side-item" href="${p.about}"><span class="i">Ôä╣´©Å</span><span>${escAttr(t.about_link || "About")}</span></a>
      <a class="side-item" href="${p.contact}"><span class="i">Ô£ë´©Å</span><span>${escAttr(t.contact_link || "Contact")}</span></a>
    </nav>

    <div class="side-divider"></div>

    <div class="side-mini">
      <div class="side-note">${escAttr((t.disclaimer || "Informational content") + " ÔÇó 18+")}</div>
    </div>
  `;
}
...
async function init(){
  ...
  setText("hero_sub", "Leitura priorit├íria do dia selecionado");
  ...
  function rerender(){
    ...
    console.log('­ƒôà renderCalendar day_key buckets:', todayMatches.length, 'today and', tomorrowMatches.length, 'tomorrow', 'active=', activeTab);
