...
window.initRadarDay = function() {
  ...
  function fetchCalendar7d() {
    return fetch('/api/v1/calendar_7d.json?force=1').then(function(r){
      if (!r.ok) throw Error('calendar_7d.json não disponível');
      return r.text();
    }).then(function(txt){
      try { return JSON.parse(txt); } catch(e) { throw Error('JSON inválido calendar_7d.json'); }
    });
  }

  (function(){
    fetchRadarDay()
      .catch(function(){ return fetchCalendar7d(); })
      .then(function(data){
        ...
        if (window.RT_TEL && typeof window.RT_TEL.record === 'function') {
          window.RT_TEL.record('radar_day_fetch_ok', { source: (data.matches ? 'radar_day' : 'calendar_7d'), count: arr.length });
        }
        renderList(arr);
        if (window.RT_TEL && typeof window.RT_TEL.record === 'function') {
