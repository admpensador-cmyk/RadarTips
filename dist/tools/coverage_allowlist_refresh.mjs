...
async function main() {
  ...
  for (const [idx, item] of items.entries()) {
    ...
    if (!candidates.length) {
      // Se não encontrou na API, mas tem league_id e display_name_override no seed, inclui manualmente
      if (item.league_id && item.display_name_override) {
        rows.push({
          region: item.region,
          country: item.country,
          league_id: item.league_id,
          display_name: item.display_name_override,
