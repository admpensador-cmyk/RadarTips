...
async function main() {
  ...
  if (!fs.existsSync(calendarFile)) {
    console.error('ERROR: calendar_7d.json not found');
    process.exit(1);
  }
  ...
  for (const match of matches) {
    try {
      const { home: homeSnapshot, away: awaySnapshot } = await generateFromCalendarMatch({
        match,
        leagueId,
        season,
      ...
      successful++;
      console.log(`[incremental] ✓ Generated snapshots for ${match.home} vs ${match.away}`);
    } catch (error) {
      failed++;
      errors.push({
  ...
  console.log(`[incremental] Successful: ${successful}`);
