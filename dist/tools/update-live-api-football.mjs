...
// Feature flag: check if LIVE is enabled in config
const config = JSON.parse(fs.readFileSync(path.join(root, "tools/api-football.config.json"), "utf8"));
if (config.live_enabled !== true) {
  console.log("⚠️  LIVE disabled in api-football.config.json (live_enabled=false)");
  console.log("   Skipping live data fetch to avoid API quota exhaustion.");
  process.exit(0);
}
